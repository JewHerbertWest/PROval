...
|---|---|---|
| Проверка подписи | Проверка подлинности внешних команд и сообщений | Используется для предотвращения подмены команд от Земли и внешних кораблей |
| Аутентификация | Проверка источника внешних команд | Используется для подтверждения доверенного источника связи |
| Валидатор команд | Проверка допустимости команд перед выполнением | Исключает выполнение опасных или некорректных операций |
| Контроль давления | Проверка давления в шлюзовой камере | Исключает открытие шлюза при опасных параметрах давления |
| Контроль герметичности | Проверка герметичности отсеков станции | Используется для предотвращения разгерметизации |
| Контроллер блокировок | Контроль одновременного открытия створок шлюза | Исключает открытие внутренней и внешней створки одновременно |
| Аварийное закрытие | Перевод шлюзовой системы в безопасное состояние | Используется при аварийных ситуациях и потере связи |
| Проверка ролей | Проверка полномочий экипажа и операторов | Используется для ограничения доступа к критическим операциям |
| Контроль зон доступа | Ограничение доступа в критические зоны станции | Исключает доступ в технические и командные отсеки без разрешения |
| Проверка телеметрии | Анализ телеметрии прибывающего корабля | Используется перед выполнением стыковки |
| Контроль стыковочного узла | Проверка состояния стыковочного оборудования | Исключает выполнение стыковки при неисправности узла |
| Детектор аварий | Выявление аварийных ситуаций | Используется для обнаружения опасных состояний станции |
| Анализ событий | Анализ системных событий и ошибок | Используется для выявления аномального поведения системы |
| Журнал внешних команд | Хранение информации о внешних командах | Используется для аудита и расследования инцидентов |
