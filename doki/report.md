...
Проект `SevastopolSHIP` представляет собой программную реализацию переработанной архитектуры системы управления шлюзами космической станции «Севастополь». Каждый значимый элемент вынесен в отдельную папку внутри `services/`.
...
Политики безопасности помогают нейтрализовать негативные сценарии.

### Негативные сценарии и защита переработанной архитектуры

| НС    | Описание сценария                                                                                                                                                 | Как участвуют политики безопасности                                                                                                                                                           | Основной защитный элемент                                                                                        |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| НС-1  | Интерфейс связи с Землёй скомпрометирован и изменяет корректную команду перед передачей дальше. Например, команда закрытия шлюза подменяется на команду открытия. | Политики безопасности не дают интерфейсу связи с Землёй напрямую обратиться к ЦСУ или сервису управления шлюзами. Команда может идти только через интерфейс фильтрации команд.                | Валидатор команд проверяет корректность команды и не пропускает изменённую команду дальше.                       |
| НС-2  | Интерфейс связи с кораблями скомпрометирован и формирует поддельный запрос на стыковку.                                                                           | Политики безопасности не дают интерфейсу связи с кораблями напрямую активировать стыковочный узел. Запрос должен пройти через фильтрацию, ЦСУ и подсистему стыковки.                          | Подтверждение стыковки не разрешает операцию, если запрос не прошёл проверку.                                    |
| НС-3  | Центральная система управления скомпрометирована и пытается открыть шлюз без проверки давления и герметичности.                                                   | Политики безопасности задают обязательный маршрут: ЦСУ должна обратиться к сервису контроля отсеков, контролю показателей и верификатору датчиков. Нельзя сразу перейти к управлению шлюзами. | Контроль показателей не подтверждает открытие шлюза без проверки давления и герметичности.                       |
| НС-4  | Сервис управления шлюзами скомпрометирован и пытается открыть внутреннюю и внешнюю створки одновременно.                                                          | Политики безопасности не дают сервису управления шлюзами напрямую управлять приводами без прохождения контроля шлюзов.                                                                        | Контроль шлюзов блокирует опасную команду, потому что две створки не должны открываться одновременно.            |
| НС-5  | Подсистема контроля доступа скомпрометирована и пытается разрешить доступ пользователю без нужных прав.                                                           | Политики безопасности требуют, чтобы решение о доступе проходило через систему аутентификации, а не принималось только подсистемой контроля доступа.                                          | Система аутентификации проверяет пользователя, роль и разрешённый сектор. Если прав нет, доступ не выдаётся.     |
| НС-6  | Сервис контроля отсеков скомпрометирован и передаёт ложные данные о нормальном давлении при опасном состоянии отсека.                                             | Политики безопасности не дают использовать данные сервиса контроля отсеков напрямую как основание для открытия шлюза. Они должны пройти через контроль показателей и верификатор датчиков.    | Контроль показателей сверяет данные с телеметрией и выявляет расхождение.                                        |
| НС-7  | Подсистема стыковки кораблей скомпрометирована и пытается подтвердить стыковку при неисправном стыковочном узле.                                                  | Политики безопасности не дают подсистеме стыковки сразу активировать стыковочный узел. Маршрут должен пройти через проверку телеметрии и контроль узла.                                       | Контроль стыковочного узла проверяет фактическое состояние узла и запрещает стыковку при неисправности.          |
| НС-8  | Центральная система управления скомпрометирована и подменяет команду закрытия шлюза на команду открытия.                                                          | Политики безопасности проверяют допустимость операции и не дают выполнить команду, которая противоречит безопасному сценарию.                                                                 | Политики безопасности выступают основным защитным элементом: они блокируют опасную операцию.                     |
| НС-9  | База состояний скомпрометирована и показывает ложный статус шлюза, например «закрыт», хотя фактически он открыт.                                                  | Политики безопасности не позволяют принимать критическое решение только по данным базы. Для операций со шлюзом должен использоваться маршрут через контроль шлюзов.                           | Контроль шлюзов сверяет состояние базы с фактическим состоянием приводов и выявляет расхождение.                 |
| НС-10 | Сервис журналирования скомпрометирован и пытается не сохранить или изменить запись о критическом событии.                                                         | Политики безопасности задают маршрут, при котором критическая запись должна пройти через контроль целостности журнала. Сервис журналирования не является единственной точкой доверия.         | Контроль целостности журнала защищает запись: делает подмену или удаление обнаруживаемыми.                       |
| НС-11 | Монитор сервисов скомпрометирован и скрывает отказ компонента или оборудования.                                                                                   | Политики безопасности не дают полагаться только на один монитор. События должны поступать в анализатор событий, который сопоставляет данные из разных источников.                             | Анализатор событий выявляет отказ по совокупности признаков и передаёт событие в аварийный контур.               |
| НС-12 | Локальный операторский терминал скомпрометирован и отправляет несанкционированную команду.                                                                        | Политики безопасности не дают терминалу напрямую обращаться к ЦСУ или исполнительным устройствам. Команда должна пройти через фильтр и проверку авторизации.                                  | Проверка авторизации блокирует команду, если у оператора нет нужных прав.                                        |
| НС-13 | Аварийный модуль скомпрометирован и не переводит систему в безопасный режим при аварии.                                                                           | Политики безопасности предусматривают отдельный аварийный маршрут через МАРЧС, чтобы система не зависела только от аварийного модуля.                                                         | МАРЧС запускает аварийное реагирование и переводит систему в безопасное состояние.                               |
| НС-14 | Интерфейс связи с Землёй скомпрометирован и повторно отправляет старую или отменённую команду.                                                                    | Политики безопасности не дают старой команде напрямую попасть к ЦСУ или шлюзам. Она должна пройти через монитор безопасности команд.                                                          | Монитор безопасности команд проверяет идентификатор команды, повторное использование и статус отмены.            |
| НС-15 | Интерфейс связи с кораблями скомпрометирован и искажает телеметрию корабля, скрывая повреждение.                                                                  | Политики безопасности не дают данным корабля напрямую стать основанием для стыковки. Телеметрия должна пройти проверку и сопоставление.                                                       | Проверка телеметрии корабля выявляет расхождение и не подтверждает стыковку.                                     |
| НС-16 | Центральная система управления скомпрометирована и формирует конфликтующие команды, например открыть шлюз и одновременно заблокировать его.                       | Политики безопасности ограничивают допустимые операции и передают такие команды через монитор безопасности команд.                                                                            | Монитор безопасности команд выявляет конфликт и не допускает одновременного исполнения взаимоисключающих команд. |
| НС-17 | Центральная система управления скомпрометирована и задерживает аварийную команду закрытия шлюза.                                                                  | Политики безопасности предусматривают аварийный маршрут, который не зависит от обычной цепочки ЦСУ.                                                                                           | МАРЧС запускает аварийное реагирование без ожидания ЦСУ, после чего система переходит в безопасный режим.        |
